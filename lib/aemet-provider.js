...
const { detectCells } = require('./aemet-cells')
...
const PRODUCTS = Object.freeze({
  PM: Object.freeze({
    ...
    description: 'AEMET single-site PPI reflectivity (Balearic Islands), ~240 km range. Display raster plus best-effort convective cells reconstructed from the reflectivity (see docs, spec §7).',
    kind: 'raster',
    units: 'dBZ',
    period: 'PT10M',
    bounds: georefBounds(),
...
class AemetRadarProvider {
  ...
  // Fetch the product's current frame GIF (cached on the frame descriptor).
  async _fetchGif (product) {
    const frame = await this._resolveFrame(product)
    if (frame.gif) return { frame, buffer: frame.gif }
    ...
    frame.gif = Buffer.from(await r.arrayBuffer())
    return { frame, buffer: frame.gif }
  }

  _decode (product, buffer) {
    if (!this._decodeRGBA) throw new Error('AEMET provider: no GIF decoder configured')
    return decodeReflectivity(buffer, this._decodeRGBA, this._staticMasks[product] || null)
  }

  // Decode a product's current frame reflectivity grid (cached on the frame).
  async _raster (product) {
    const { frame, buffer } = await this._fetchGif(product)
    if (frame.raster) return frame.raster
    frame.raster = this._decode(product, buffer)
    return frame.raster
  }

  // --- Inference tier (best-effort, reconstructed from a rendered product) ---
  // The AEMET regional product is a rendered palette image, so the "raw" bytes
  // returned here are the GIF itself and the cells below are reconstructed from
  // palette-decoded, legend-quantised reflectivity (see lib/aemet-cells.js and
  // docs/aemet-provider.md, and radar-provider-specification.md §7). This is a
  // documented best-effort for convective-cell awareness, not native raw data.
  async downloadRaw (product, epochMs) {
    this._assertProduct(product)
    const { frame, buffer } = await this._fetchGif(product)
    return {
      buffer,
      key: `aemet-${product.toLowerCase()}-${Number(epochMs) || frame.epochMs}.gif`,
      contentType: 'image/gif'
    }
  }

  rawExtension () { return '.gif' }

  async cellsFromRaw (product, buffer) {
    this._assertProduct(product)
    const raster = this._decode(product, buffer)
    return detectCells(raster, {
      thresholdDbz: this.cfg.cellThresholdDbz,
      minPixels: this.cfg.cellMinPixels
    })
  }
